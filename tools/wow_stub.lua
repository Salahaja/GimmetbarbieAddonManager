...
-- Real parenting and real child enumeration: an addon that walks
-- Minimap:GetChildren() and re-parents what it finds is doing its actual work
-- here, so no-ops would make any test of it meaningless.
function frameMethods:SetParent(p) self._parent = p end
function frameMethods:GetChildren()
    local kids = {}
    for _, f in ipairs(Stub.allFrames) do
        if f._parent == self then table.insert(kids, f) end
    end
    return unpack(kids)
end
...
for _, name in ipairs({
    ...
    "SetHighlightTexture", "SetPushedTexture", "SetID", "SetAllPoints",
